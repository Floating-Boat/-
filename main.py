...
def model_train(model, loadweights):
    learning_rate = LearningRateScheduler(lrschedule)
    """
    ...
    每个训练期后保存模型
    """
    mdcheck = ModelCheckpoint(weight_path, monitor='val_accuracy', save_best_only=True)
    td = TensorBoard(log_dir='D:/code/python_code/大创/tensorboard_log')
    """
    ...
    """
    SGD(learning_rate=0.01, momentum=0.0, decay=0.0, nesterov=False)
    学习率，用于加速SGD相关方向前进，学习率衰减值，是否使用Nesterov动量
    随机梯度下降优化器
    """
    # rmsprot = RMSprop(learning_rate=0.001)
    sgd = SGD(learning_rate=0.1, momentum=0.9, decay=5e-4, nesterov=True)
    ...
    history = model.fit(train_generator,
                        ...
                        callbacks=[learning_rate, mdcheck, td])
...
def draw_loss_acc(history):
    ...
    plt.plot(x_trick, val_accuracy, 'y-', label='val_accuracy')
...
if __name__ == "__main__":
    train_path = "E:\\大学作业\\23大创\\资料\\shuju\\train" # 训练集路径
    val_path = "E:\\大学作业\\23大创\\资料\\shuju\\test1"    # 验证集路径
    test_path = "E:\\大学作业\\23大创\\资料\\shuju\\test2"       # 测试集路径
    num_calsses = 54                                         # 分类种类，改
    batch_size = 128
...
def upload_file(f):
    img = Image.open(f)
    plt.imshow(img)
    path = "D:/code/python_code/大创/image_test"
    name = time.time()
...
if __name__ == "__main__":
    ...
    image_path = 'D:/code/python_code/大创/image_test'
    train_path = "D:/code/python_code/大创/shuju/train"
    model = Alex_model(54)          #改
    model.load_weights(weight_path)
    ...
    img = Image.open("D:/code/python_code/大创/back.jpg")
    (x, y) = img.size
    x //= 4
    y //= 4
    img = img.resize((x, y))
